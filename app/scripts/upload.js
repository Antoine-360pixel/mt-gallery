...
$(document).ready(function() {
	...
	$('#mt-drop-files, #mt-upload-input').bind('drop', function(e) {
		...
		$.each(files, function(index, file) {
			...
			if (!files[index].type.match('image.*')) {
				...
				if(errMessage == 0) {
					$('#mt-drop-files').html('Hey! Images only');
					++errMessage
				}
				else if(errMessage == 1) {
					$('#mt-drop-files').html('Stop it! Images only!');
					++errMessage
				}
				else if(errMessage == 2) {
					$('#mt-drop-files').html("Can't you read?! Images only!");
					++errMessage
				}
				else if(errMessage == 3) {
					$('#mt-drop-files').html("Fine! Keep dropping non-images.");
					errMessage = 0;
				}
				return false;
			}
